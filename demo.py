    ...
    def _process_detections(self, frame, results, current_time):
        ...
        if results.boxes is not None and results.boxes.id is not None:
            ...
            for box, track_id, conf in zip(boxes, track_ids, confidences):
                ...
                # Professional color scheme (gradient based on time)
                if time_in_restaurant < 300:  # < 5 minutes
                    box_color = (80, 200, 120)  # Mint green
                elif time_in_restaurant < 1800:  # < 30 minutes
                    box_color = (100, 180, 255)  # Soft orange
                else:
                    box_color = (120, 120, 255)  # Soft red
                
                # Draw rounded corner bounding box with gradient effect
                self._draw_rounded_rectangle(annotated_frame, (x1, y1), (x2, y2), box_color, 3, 15)
                ...
                time_label = f"Duration: {time_str}"
                conf_label = f"Conf: {conf*100:.0f}%"
                
                # Draw professional info panel above bounding box
                ...
                corner_thickness = 3
                # Top-left
                cv2.line(annotated_frame, (x1, y1), (x1 + corner_length, y1), box_color, corner_thickness)
                cv2.line(annotated_frame, (x1, y1), (x1, y1 + corner_length), box_color, corner_thickness)
                # Top-right
                cv2.line(annotated_frame, (x2, y1), (x2 - corner_length, y1), box_color, corner_thickness)
                cv2.line(annotated_frame, (x2, y1), (x2, y1 + corner_length), box_color, corner_thickness)
                # Bottom-left
                cv2.line(annotated_frame, (x1, y2), (x1 + corner_length, y2), box_color, corner_thickness)
                cv2.line(annotated_frame, (x1, y2), (x1, y2 - corner_length), box_color, corner_thickness)
                # Bottom-right
                cv2.line(annotated_frame, (x2, y2), (x2 - corner_length, y2), box_color, corner_thickness)
                cv2.line(annotated_frame, (x2, y2), (x2, y2 - corner_length), box_color, corner_thickness)
                
                # Store track history (center point)
                center = ((x1 + x2) // 2, (y1 + y2) // 2)
                self.track_history[track_id].append(center)
                
                # Draw smooth trajectory with gradient
                if len(self.track_history[track_id]) > 1:
                    points = np.array(self.track_history[track_id][-30:], dtype=np.int32)
                    for i in range(len(points) - 1):
                        alpha = (i + 1) / len(points)
                        thickness = int(2 + alpha * 2)
                        cv2.line(annotated_frame, tuple(points[i]), tuple(points[i + 1]), 
...
def main():
    ...
    # Process video
    video_path = 'input1.mp4'  # Replace with your video path
    output_path = 'tracked_output1.mp4'   # Optional: save annotated video
